...
const MissionLeftSidebar = (props: Props) => {
  ...
  if (sidebarType == "asset") {
    return (
      <Layout>
        <AssetDetail key={sidebarData?.id} />
      </Layout>
    );
  }
...
const Layout = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="min-w-80 max-w-80 h-screen p-2">
      <div className="bg-card h-full rounded-md">{children}</div>
    </div>
  );
};
  ...
  return (
    <div className="overflow-y-auto">
      <Command className="h-full">
        <CommandInput
          placeholder="Type a entity name"
              ...
              <Checkbox
                ...
                onCheckedChange={(checked) => {
                  if (checked) {
                    ...
                    setRenderEntities(
                      renderEntities.filter((entity) => entity !== item.value)
                    );
                  }
                }}
              />
        ...
        <CommandList className="h-[77vh]">
          ...
          {renderEntities.includes("assets") && (
            <CommandGroup heading={"Assets (" + mission.assets?.length + ")"}>
              <AssetList assets={mission.assets ? mission.assets : []} />
            </CommandGroup>
          )}
          <CommandSeparator />
          {renderEntities.includes("geofences") && (
            <CommandGroup
              heading={"Geofences (" + mission.geofences?.length + ")"}
            >
              <GeofenceList
                geofences={mission.geofences ? mission.geofences : []}
              />
            </CommandGroup>
          )}
          <CommandSeparator />
